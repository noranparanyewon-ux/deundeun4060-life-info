...
describe("든든한 4060 생활정보 콘텐츠 구조", () => {
  ...
  it("includes a complete mobile quick-menu route set without placeholder anchors", () => {
    ...
    expect(styleSource).toContain(".main-nav a { display: inline-flex; align-items: center; gap: 5px; padding-top: 29px; padding-bottom: 26px; font-size: 16px; font-weight: 650; }");
    ...
    expect(appSource).toContain("MapPin");
    expect(appSource).toContain('articlePrefix: "/welfare/"');
    expect(styleSource).toContain(".mobile-menu-group a.is-active");
