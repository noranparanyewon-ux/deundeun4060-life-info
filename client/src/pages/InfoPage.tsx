...
const content = {
  ...
  contact: {
    kicker: "의견을 보내는 곳",
    title: "더 정확한 안내를\n함께 만들어 주세요.",
    intro: "글에서 사실과 다른 부분을 발견했거나, 다음에 다뤘으면 하는 생활 질문이 있다면 안전한 문의 방법을 이 페이지에서 안내합니다.",
  },
  privacy: {
    kicker: "개인정보처리방침",
    title: "필요한 만큼만,\n분명하게 다룹니다.",
    intro: "현재 기본 웹 주소에서 운영하는 초기 버전의 안내문입니다. 실제로 수집하는 정보와 연결하는 분석·광고 서비스가 확정되면 운영 방식에 맞춰 내용을 업데이트합니다.",
  },
  disclaimer: {
    kicker: "면책 안내",
    title: "정보는 출발점이고,\n최종 확인은 공식 안내에서.",
...
export default function InfoPage({ type }: { type: InfoType }) {
  ...
  return (
    <>
      ...
      <div className="container breadcrumb"><Link href="/">홈</Link><span>/</span><span>{current.kicker}</span></div>
      <section className="info-body container">
        ...
        {type === "contact" && <div className="info-single"><div className="info-icon"><Mail size={25} /></div><span className="section-kicker">문의 안내</span><h2>안전한 문의 방법을 안내합니다.</h2><p>운영에 사용할 공식 이메일 주소를 확정하면 스팸과 개인정보 노출을 줄일 수 있도록 이 페이지에 문의 방법을 안내합니다.</p><p>정정 요청을 보낼 때에는 해당 글의 주소와 문제가 되는 문장을 함께 적고, 주민등록번호·계정 비밀번호·인증번호와 같은 민감한 정보는 보내지 마세요.</p><div className="info-callout"><strong>문의 전 확인</strong><span>공식 이메일 주소를 정한 뒤 이 페이지와 푸터의 문의 경로를 실제 운영 방식에 맞춰 업데이트합니다.</span></div></div>}
        {type === "privacy" && <div className="info-single legal-copy"><div className="info-icon"><ShieldCheck size={25} /></div><span className="section-kicker">초기 운영 안내</span><h2>실제 운영 방식에 맞춰 관리합니다.</h2><p>현재 기본 버전에는 회원가입, 댓글, 문의 입력폼이 없습니다. 따라서 방문자가 입력한 개인정보를 사이트 자체에 저장하는 기능도 없습니다.</p><p>문의 폼, 분석 도구, 광고 서비스 등을 추가하면 수집 항목, 이용 목적, 보관 기간, 제3자 제공 여부, 이용자 권리와 문의 방법을 실제 설정에 맞춰 다시 작성합니다. 해당 문서는 서비스 연결 전에 검토하고 공개합니다.</p><div className="legal-date"><span>최종 업데이트</span><strong>2026년 8월 26일</strong></div></div>}
