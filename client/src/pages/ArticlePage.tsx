...
export default function ArticlePage() {
  ...
  return (
    <>
      <SEO title={`${article.title} | 든든한 4060 생활정보`} description={article.excerpt} />
      ...
      <header className="article-header container"><div className="article-header__main"><span className="category-label" style={{ "--label-accent": category.accent } as React.CSSProperties}>{category.label}</span><h1>{article.title}</h1><p className="article-excerpt">{article.excerpt}</p><div className="article-meta article-meta--large"><span><Clock3 size={15} /> {article.readingTime}</span><span>업데이트 {article.updated}</span></div></div>{article.image && <div className="article-header__image"><img src={article.image} alt="" /></div>}</header>
