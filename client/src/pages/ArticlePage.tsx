...
export default function ArticlePage() {
  ...
  return (
    <>
      ...
      {related.length > 0 && <section className="related-section container"><div className="section-heading-row"><div><span className="section-kicker">이 서랍에서 더 읽기</span><h2>다음으로 이어지는 글</h2></div><Link href={`/category/${category.slug}`} className="section-link">주제 전체 보기 <ArrowLeft size={16} /></Link></div><div className="related-grid">{related.map((item) => <Link key={item.slug} href={item.canonicalPath} className="related-card">{item.image && <img src={item.image} alt={item.imageAlt} loading="lazy" decoding="async" />}<div><span className="category-label" style={{ "--label-accent": category.accent } as React.CSSProperties}>{category.label}</span><h3>{item.title}</h3><p>{item.excerpt}</p><span className="read-more">읽어보기 <ArrowUpRight size={15} /></span></div></Link>)}</div></section>}
      </article>
    </>
  );
}
