...
export default function LegacyDigitalPage() {
  ...
  return <>
    ...
    <article className="article-page">
      ...
      <header className="article-header container"><div className="article-header__main"><span className="category-label" style={{ "--label-accent": "#466B76" } as React.CSSProperties}>기존 디지털 자료</span><h1>{article.title}</h1><p className="article-excerpt">{article.excerpt}</p><div className="article-meta article-meta--large"><span><Clock3 size={15} /> {article.readingTime}</span><span>업데이트 {article.updated}</span><span className="verified-meta">보존 자료</span></div></div>{article.image && <div className="article-header__image"><img src={article.image} alt="" /></div>}</header>
